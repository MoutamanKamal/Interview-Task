...
function wave(canvas, targetPlane) {
  console.log('wave'); // track the mouse positions to send it to the shaders
  ...
  let webGLCurtain = new Curtains(canvas); // get the plane element

  let planeElement = document.querySelector(targetPlane); // set the initial parameters (basic uniforms)
  ...
  let plane = webGLCurtain.addPlane(planeElement, params); // if the plane has been successfully created we could start listening to mouse/touch events and update its uniforms

  plane && plane.onReady(function () {
    // set a field of view of 35 to exaggerate perspective
    plane.setPerspective(35); // listen the mouse/touch events on the whole document
  ...
  function handleMovement(e, plane) {
    ...
    let mouseCoords = plane.mouseToPlaneCoords(mousePosition.x, mousePosition.y); // update the mouse position uniform

    plane.uniforms.mousePosition.value = [mouseCoords.x, mouseCoords.y]; // reassign mouse strength
